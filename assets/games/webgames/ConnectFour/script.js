...
function main() {
  ...
  const displayCurrentPLayer = getid("currentPlayer");
  ...
  for (let i = 0; i < squares.length; i++) {
    squares[i].addEventListener("click", () => {
      //If the square below is taken, you get on top of it
      if (
        ...
        if (currentPlayer === 1) {
          ...
          currentPlayer = 2;
          displayCurrentPLayer.textContent = "Computer";
        } else {
          squares[i].classList.add("taken");
          squares[i].classList.add("playerTwo");
          currentPlayer = 1;
          displayCurrentPLayer.textContent = "User";
        }
      } else {
